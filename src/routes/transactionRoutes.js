...
transactionRoutes.post('/create', async (req, res) => {
  ...
  if (!xRefPayco || !xdescription || !xresponse || !productIds || productIds.length === 0) {
    return res.status(400).json({ error: 'Faltan campos obligatorios o productIds está vacío' });
  }

  try {
    // Aquí puedes trabajar con productIds como un array
